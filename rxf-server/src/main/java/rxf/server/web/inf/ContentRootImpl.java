...
public class ContentRootImpl extends Impl implements PreRead{
  ...
  public static String fileScrub(String scrubMe){
    ...
    return (null==scrubMe)?null:scrubMe.trim().replace(inverseChar,File.separatorChar).replace(DOUBLESEP,""+File.separator)
        .replace("..",".");
  }
  ...
  public void onRead(SelectionKey key) throws Exception{
    ...
    req=(HttpRequest)new Rfc822HeaderState()
        .addHeaderInterest(Accept$2dEncoding,If$2dModified$2dSince,If$2dUnmodified$2dSince).$req().apply((ByteBuffer)flip);
  ...
  public void onWrite(SelectionKey key) throws Exception{
    ...
    if(null!=since){
      ...
      if(null!=since){
        ...
        if(cachedDate.before(fdate)){
          ...
          res.status(HttpStatus.$412).headerString(Connection,"close").headerString(Last$2dModified,
              DateHeaderParser.formatHttpHeaderDate(fdate));
          int write=channel.write(res.as(ByteBuffer.class));
          key.interestOps(OP_READ).attach(null);
          return;
    ...
    if(send200){
      ...
      res.status(HttpStatus.$200).headerString(Content$2dType,((null==mimeType)?MimeType.bin:mimeType).contentType)
          .headerString(Content$2dLength,String.valueOf(length)).headerString(Connection,"close").headerString(Date,
              DateHeaderParser.formatHttpHeaderDate(fdate));;
      if(null!=ceString)
      ...
      key.interestOps(OP_WRITE).attach(new Impl(){
        ...
        public void onWrite(SelectionKey key) throws Exception{
          int write=channel.write(req.$res().status(HttpStatus.$404).headerString(Content$2dLength,"0").as(ByteBuffer.class));
          key.selector().wakeup();
          key.interestOps(OP_READ).attach(null);
        }
      });
