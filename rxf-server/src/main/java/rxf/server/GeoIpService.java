...
public class GeoIpService {
	...
	public static void createGeoIpIndex() throws IOException,
		...
		System.err
				.println(MessageFormat
						.format(
								"download complete in {0} archive size: {1,number,#.##}Mb @{2,number,#.##} Mb/s",
								v, i / MEG, i / v));
		...
		try {
			EXECUTOR_SERVICE.submit(new Callable() {
				public Object call() throws Exception {
					String fire = null;
					try {
						fire = RevisionFetch.$().db("geoip").docId("current")
								.to().fire().json();
					...
					if (null == fire) {
						...
						CouchTx geoip = DbCreate.$().db("geoip").to().fire()
								.tx();
						assert geoip.ok();
						//...
						CouchTx tx = DocPersist.$().db("geoip")
								.docId("current").to().fire().tx();
						assert tx.ok();
						createGeoIpIndex();
	...
	public static ByteArrayOutputStream downloadMaxMindBinaryTarXz(String href)
			throws IOException {
		long l = System.currentTimeMillis();
	...
	public static Triple<Integer[], ByteBuffer, ByteBuffer> buildGeoIpFirstPass(
			ByteArrayOutputStream archiveBuffer) throws IOException {
		TarArchiveInputStream tarArchiveInputStream = new TarArchiveInputStream(
				new ByteArrayInputStream(archiveBuffer.toByteArray()));
