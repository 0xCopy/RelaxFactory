...
import static rxf.server.BlobAntiPatternRelic.getDefaultOrgName;
...
public class CouchServiceFactory {
  ...
  private static class CouchServiceHandler<E> implements InvocationHandler, CouchNamespace<E> {
    AtomicReference<Map<String, String>> viewMethods =
        new AtomicReference<Map<String, String>>(new TreeMap<String, String>());
    private Future init;
    Class<E> entityType;

    public CouchServiceHandler(Class<?> serviceInterface, String... ns) throws ExecutionException,
    ...
    void init(final Class<?> serviceInterface, final String... initNs) throws ExecutionException,
        InterruptedException {
      ...
      init = RelaxFactoryServer.App.get().getEXECUTOR_SERVICE().submit(new Callable() {
        ...
        public Object call() throws Exception {
          ...
          try {
            ...
            try {
              design.version =
                  RevisionFetch.$().db(getPathPrefix()).docId(design.id).to().fire().json();
            ...
            final String stringParam = CouchDriver.GSON.toJson(design);
    ...
    public Object invoke(Object proxy, final Method method, final Object[] args)
      ...
      if (viewMethods.get().containsKey(method.getName())) {
        return RelaxFactoryServer.App.get().getEXECUTOR_SERVICE().submit(new Callable() {
          public Object call() throws Exception {
            ...
            if (args != null) {//apparently args is null for a zero-arg method
              ...
              for (int i = 0; i < args.length; i++) {
                jsonArgs[i] = URLEncoder.encode(CouchDriver.GSON.toJson(args[i]), "UTF-8");
              }
            }
        ...
        if ("persist".equals(method.getName())) {
          ...
          String doc =
              CouchDriver.DocFetch.$().db(getPathPrefix()).docId((String) args[0]).to().fire()
                  .json();
