...
public class CouchChangesClient extends AsioVisitor.Impl {
	...
	public String getFeedString() {
		return "/" + feedname
				+ "/_changes?include_docs=true&feed=continuous&heartbeat="
				+ POLL_HEARTBEAT_MS;
	}
			...
			if (s.startsWith("HTTP/1.1 20")) {
				...
				key.interestOps(OP_WRITE).attach(new Object[]{new Impl() {
					...
					public void onRead(SelectionKey key) throws Exception {
						...
						Object prev = attachment.length > 2
								? attachment[2]
								: null;
						boolean stuff = false;
						...
						if (stuff) {
							...
							EXECUTOR_SERVICE.submit(new UpdateStreamRecvTask(
									objects));
						}
					}
				}, getFeedString()});
	...
	public class UpdateStreamRecvTask implements Runnable {
		...
		public void run() {
			...
			do {
				...
				b.flip();
				Integer integer = Integer.valueOf(UTF8.decode(b).toString()
						.trim(), 0x10);
