...
public enum CouchMetaDriver {
							...
							switch (httpStatus) {
								...
								case $201 :
									...
									if (remaining == cursor.remaining()) {
										...
										cursor = ByteBuffer.allocate(remaining)
												.put(cursor);
									}
									break;
								default : //error
		...
		public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder,
			...
			enqueue(channel, OP_CONNECT | OP_WRITE, new Impl() {
				...
				ByteBuffer header = (ByteBuffer) request.path(
						BlobAntiPatternObject.scrub("/" + db
								+ (null == id ? "" : "/" + id))).method(GET)
						.addHeaderInterest(STATIC_CONTENT_LENGTH_ARR).as(
								ByteBuffer.class);
	...
	DocPersist {
		public ByteBuffer visit(DbKeysBuilder dbKeysBuilder,
			...
			String sb = BlobAntiPatternObject.scrub('/'
					+ db
					+ (null == docId ? "" : '/' + docId
							+ (null == rev ? "" : "?rev=" + rev)));
	...
	DocDelete {
		public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder,
			...
			enqueue(channel, OP_WRITE | OP_CONNECT, new Impl() {
				...
				private HttpResponse response;
				ByteBuffer header = (ByteBuffer) request.path(
						BlobAntiPatternObject.scrub("/" + dbKeysBuilder.get(db)
					...
					if (null == cursor) {
						...
						if (BlobAntiPatternObject.suffixMatchChunks(
							...
							if (remaining == cursor.remaining()) {
								...
								cursor = ByteBuffer.allocate(remaining).put(
										cursor);
							}
						}
					} else {
	...
	ViewFetch {
		public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder,
			...
			final AtomicReference<ByteBuffer> payload = new AtomicReference<ByteBuffer>();
			final CyclicBarrier cyclicBarrier = new CyclicBarrier(2);
			final String db = BlobAntiPatternObject
					.scrub('/' + (String) dbKeysBuilder.get(etype.db));
			...
			enqueue(channel, OP_WRITE | OP_CONNECT, new Impl() {
				...
				public void onWrite(SelectionKey key) throws Exception {
					...
					header = (ByteBuffer) request.method(GET).path(
							BlobAntiPatternObject.scrub('/' + db + '/'
									+ dbKeysBuilder.get(view))).headerString(
							Accept, MimeType.json.contentType).as(
							ByteBuffer.class);
							...
							switch (httpStatus) {
								case $200 :
									...
									if (response.headerStrings().containsKey(
										...
										final String remainingString = response
												.headerString(Content$2dLength);
					...
					EXECUTOR_SERVICE.submit(new Callable() {
						public Object call() throws Exception {
							...
							while (sum > 0 && src.hasRemaining()) {
								...
								final int i = Integer.parseInt(UTF8.decode(
	...
	JsonSend {
		public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder,
			...
			final AtomicReference<ByteBuffer> payload = new AtomicReference<ByteBuffer>();
			final CyclicBarrier cyclicBarrier = new CyclicBarrier(2);
			String opaque = BlobAntiPatternObject
					.scrub('/' + (String) dbKeysBuilder.get(etype.opaque));
			...
			final HttpMethod method = 1 == slashCounter
					|| !(lastSlashIndex < opaque.lastIndexOf('?') && lastSlashIndex != opaque
							.indexOf('/')) ? POST : PUT;
			HttpRequest request = state.$req();
			final ByteBuffer header = (ByteBuffer) request.method(method).path(
					opaque).headerInterest(STATIC_JSON_SEND_HEADERS)
					.headerString(Content$2dLength,
