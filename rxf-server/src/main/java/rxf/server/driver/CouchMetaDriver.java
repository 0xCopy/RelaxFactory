...
public enum CouchMetaDriver {
    ...
    @DbKeys({db})
    DbCreate {
        public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder,
                                final ActionBuilder actionBuilder) throws Exception {
            ...
            enqueue(channel, OP_WRITE | OP_CONNECT, new Impl() {
                ...
                ByteBuffer header = (ByteBuffer) request.method(PUT).path(
                        "/" + db)
                        //          .headerString(Content$2dLength, "0")
                        .as(ByteBuffer.class);
                ...
                public void onWrite(SelectionKey key) throws Exception {
                    ...
                    header.clear();
                    response = request.headerInterest(STATIC_JSON_SEND_HEADERS)
                            .$res();
                ...
                public void onRead(SelectionKey key) throws Exception {
                    if (null == cursor) {
                        ...
                        header = null == header
                                ? ByteBuffer
                                .allocateDirect(getReceiveBufferSize())
                                : header.hasRemaining() ? header : ByteBuffer
                                .allocateDirect(header.capacity() * 2)
                                .put((ByteBuffer) header.flip());

                        int read = channel.read(header);
                        ByteBuffer flip = (ByteBuffer) header.duplicate()
                                .flip();
                        ...
                        if (BlobAntiPatternObject.suffixMatchChunks(
                            ...
                            if (DEBUG_SENDJSON) {
                                System.err.println(deepToString(response
                                        .statusEnum(), response, UTF8
                                        .decode((ByteBuffer) cursor.duplicate()
                                                .rewind())));
                            }
                            ...
                            switch (httpStatus) {
                                ...
                                case $201:
                                    ...
                                    if (remaining == cursor.remaining()) {
                                        ...
                                        cursor = ByteBuffer.allocateDirect(
    ...
    DbDelete {
        public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder,
            ...
            enqueue(channel, OP_WRITE | OP_CONNECT, new Impl() {
                ...
                ByteBuffer header = (ByteBuffer) request.method(DELETE)
                        .pathResCode("/" + dbKeysBuilder.get(db)).as(
                                ByteBuffer.class);
                ByteBuffer cursor;
        ...
        public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder,
            ...
            enqueue(channel, OP_CONNECT | OP_WRITE, new Impl() {
                ...
                ByteBuffer header = (ByteBuffer) request.path(
                        scrub("/" + db + (null == id ? "" : "/" + id))).method(
                        GET).addHeaderInterest(STATIC_CONTENT_LENGTH_ARR).as(
                        ByteBuffer.class);
    ...
    RevisionFetch {
        public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder,
            ...
            enqueue(channel, OP_CONNECT | OP_WRITE, new Impl() {
                ...
                String id = (String) dbKeysBuilder.get(docId);
                HttpRequest request = actionBuilder.state().$req();
                final String scrub = scrub("/" + db
                        + (null != id ? "/" + id : ""));
                ByteBuffer header = (ByteBuffer) request.path(scrub).method(
                        HEAD).as(ByteBuffer.class);
                public HttpResponse response;
    ...
    DocPersist {
        public ByteBuffer visit(DbKeysBuilder dbKeysBuilder,
            ...
            actionBuilder.state().$req().headerString(
                    HttpHeaders.Content$2dType, MimeType.json.contentType);
    ...
    DocDelete {
        public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder,
            ...
            enqueue(channel, OP_WRITE | OP_CONNECT, new Impl() {
                ...
                private HttpResponse response;
                ByteBuffer header = (ByteBuffer) request.path(
                        scrub("/" + dbKeysBuilder.get(db) + "/"
                                + dbKeysBuilder.get(docId) + "?rev="
                                + dbKeysBuilder.get(rev))).method(DELETE).as(
                        ByteBuffer.class);
                ByteBuffer cursor;
                ...
                public void onWrite(SelectionKey key) throws Exception {
                    ...
                    header.clear();
                    response = request
                            .headerInterest(STATIC_CONTENT_LENGTH_ARR).$res();
                    ...
                    if (null == cursor) {
                        ...
                        if (BlobAntiPatternObject.suffixMatchChunks(
                            ...
                            if (remaining == cursor.remaining()) {
                                ...
                                cursor = ByteBuffer.allocateDirect(remaining)
                                        .put(cursor);
                            }
                        }
                    } else {
        ...
        public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder,
            ...
            enqueue(channel, OP_WRITE | OP_CONNECT, new Impl() {
                ...
                public void onWrite(SelectionKey key) throws Exception {
                    ...
                    header = (ByteBuffer) request.method(GET).path(
                            scrub('/' + db + '/' + dbKeysBuilder.get(view)))
                ...
                public void onRead(SelectionKey key) throws IOException {
                        ...
                        if (BlobAntiPatternObject.suffixMatchChunks(
                            ...
                            switch (httpStatus) {
                                case $200:
                                    ...
                                    if (response.headerStrings().containsKey(
                                            Content$2dLength.getHeader())) { //rarity but for empty rowsets
                                        String remainingString = response
                                                .headerString(Content$2dLength);
                                        final int remaining = Integer
                                                .parseInt(remainingString);
                                        if (cursor.remaining() == remaining) {
                                            ...
                                            //windows workaround?
                                            key.attach(new Impl() {
                                                private ByteBuffer cursor1 = cursor
                                                        .capacity() > remaining
                                                        ? (ByteBuffer) cursor
                                                        .limit(remaining)
                                                        : ByteBuffer
                                                ...
                                                public void onRead(
                                                    ...
                                                    if (!cursor1.hasRemaining()) {
                                                        ByteBuffer flip1 = (ByteBuffer) cursor1
                                                                .flip();
                                                        simpleDeploy(flip1);
                                                    }
                                                }
                    ...
                    boolean suffixMatches = BlobAntiPatternObject
                            .suffixMatchChunks(CE_TERMINAL, cursor.duplicate(),
                ...
                private void deliver() {
                    ...
                    EXECUTOR_SERVICE.submit(new Callable() {
                        public Object call() throws Exception {
                            ...
                            ByteBuffer src = ((ByteBuffer) outbound.rewind())
                                    .duplicate();
                            int endl = 0;
                            while (sum > 0 && src.hasRemaining()) {
                                if (DEBUG_SENDJSON)
                                    System.err.println("outbound:----\n"
                                            + UTF8.decode(outbound.duplicate())
                                            .toString() + "\n----");
                                ...
                                int i = Integer.parseInt(UTF8.decode(
                                        (ByteBuffer) src.duplicate().flip())
                                        .toString().trim(), 0x10);
                                src = ((ByteBuffer) src.compact().position(i))
                                        .slice();
                                endl += i;
                                sum -= i;
                                if (0 == i)
                            ...
                            ByteBuffer retval = (ByteBuffer) outbound.clear()
                                    .limit(endl);
    ...
    JsonSend {
        public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder,
            ...
            final AtomicReference<ByteBuffer> payload = new AtomicReference<ByteBuffer>();
            final CyclicBarrier cyclicBarrier = new CyclicBarrier(2);
            String opaque = scrub('/' + (String) dbKeysBuilder
                    .get(etype.opaque));
            ...
            ByteBuffer header = (ByteBuffer) request.method(method)
                    .path(opaque).headerInterest(STATIC_JSON_SEND_HEADERS)
                    .headerString(Content$2dLength,
                            String.valueOf(outbound.length)).headerString(
            ...
            enqueue(channel, OP_WRITE | OP_CONNECT, new Impl() {
                ...
                ByteBuffer header = (ByteBuffer) request
                        ...
                        .headerString(Content$2dLength,
                                String.valueOf(outbound.length))
                        .headerString(Accept, MimeType.json.contentType)
    ...
    BlobSend {
        public ByteBuffer visit(DbKeysBuilder dbKeysBuilder,
            ...
            final CyclicBarrier cyclicBarrier = new CyclicBarrier(2);
            final HttpRequest request = actionBuilder.state().$req();
            final ByteBuffer payload = (ByteBuffer) dbKeysBuilder
                    .<ByteBuffer>get(etype.blob).rewind();
            String x = null;

            for (Object o : new Object[]{dbKeysBuilder.get(etype.mimetypeEnum),
                    dbKeysBuilder.get(etype.mimetype), MimeType.bin}) {
                if (null != o) {
            ...
            final String sb = scrub('/' + db + '/' + docId + '/'
                    + URLEncoder.encode(attachname, UTF8.displayName())
                    + "?rev=" + rev);
            ...
            enqueue(channel, OP_WRITE, new Impl() {
                ...
                public void onWrite(SelectionKey key) throws Exception {
                    ...
                    ByteBuffer as = request.method(PUT).path(sb).headerString(
                            Expect, "100-Continue").headerString(
                            Content$2dType, ctype).headerString(Accept,
                            MimeType.json.contentType).headerString(
                            Content$2dLength, String.valueOf(limit)).as(
                            ByteBuffer.class);
                ...
                public void onRead(SelectionKey key) throws Exception {
                    ...
                    Rfc822HeaderState apply = httpResponse
                            .apply((ByteBuffer) byteBuffer[0].flip());
                    HttpStatus httpStatus = httpResponse.statusEnum();
                    switch (httpStatus) {
                        case $100:
                            key.interestOps(OP_WRITE).attach(new Impl() {
                                public HttpResponse response = (HttpResponse) request
                                        .$res()
                                        .headerInterest(Content$2dLength);
                                public ByteBuffer cursor;
    ...
    public String builder() throws NoSuchFieldException {
        ...
        if (field.getType().isAssignableFrom(CouchMetaDriver.class)) {
            ...
            etype[] optionalParams = field.getAnnotation(DbKeys.class)
                    .optional();
            ...
            if (null != annotation) {
                ...
                s = s.replace(FIRE_METHODS, t).replace(IFACE_FIRE_TARGETS,
                        iface);
            }

            s = s.replace(GENERATED_METHODS, s1).replace("_ename_", name())
                    .replace(PCOUNT, String.valueOf(vl));
        }
        return s;
    }

    private String writeParameterSetter(String rtypeTypeParams, String s1,
                                        etype etype, Class<?> clazz) {
        @Language("JAVA")
        String y = "public _ename_" + rtypeTypeParams
                + "  _name_(_clazz_ _sclazz_){parms.put(DbKeys.etype."
                + etype.name() + ",_sclazz_);return this;}\n";
        s1 += y.replace("_name_", etype.name()).replace("_clazz_",
                clazz.getCanonicalName()).replace("_sclazz_",
