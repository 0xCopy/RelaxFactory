...
public enum CouchMetaDriver {
    ...
    DbCreate {
        public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder, final ActionBuilder actionBuilder) throws Exception {
            ...
            RelaxFactoryServer.App.get().enqueue(channel, OP_WRITE | OP_CONNECT, new Impl() {
                // *******************************
                // *******************************
                ...
                public void onRead(SelectionKey key) throws Exception {
                    if (null == cursor) {
                        ...
                        header = null == header ? ByteBuffer.allocateDirect(getReceiveBufferSize()) : header.hasRemaining() ? header : ByteBuffer
                                .allocateDirect(header.capacity() * 2).put((ByteBuffer) header.flip());

                        int read = channel.read(header);
                        ByteBuffer flip = (ByteBuffer) header.duplicate().flip();
                        response.apply((ByteBuffer) flip);

                        if (suffixMatchChunks(HEADER_TERMINATOR, response.headerBuf())) {
                            cursor = (ByteBuffer) flip.slice();
                            header = null;

                            if (d()) {
                                System.err.println(deepToString(response.statusEnum(), response, UTF8.decode((ByteBuffer) cursor.duplicate()
                                        .rewind())));
                            }
    ...
    DbDelete {
        public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder, final ActionBuilder actionBuilder) throws Exception {
            ...
            RelaxFactoryServer.App.get().enqueue(channel, OP_WRITE | OP_CONNECT, new Impl() {
                ...
                ByteBuffer header = (ByteBuffer) request.method(DELETE).pathResCode("/" + dbKeysBuilder.get(db)).as(
                        ByteBuffer.class);
                ByteBuffer cursor;
    ...
    DocFetch {
        public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder, final ActionBuilder actionBuilder) throws Exception {
            ...
            final SocketChannel channel = createCouchConnection();
            final CyclicBarrier cyclicBarrier = new CyclicBarrier(2);
            RelaxFactoryServer.App.get().enqueue(channel, OP_CONNECT | OP_WRITE, new Impl() {
                // *******************************
                // *******************************
                // pathological buffersize traits
                // *******************************
                ...
                HttpRequest request = actionBuilder.state().$req();
                private HttpResponse response;
                ByteBuffer header = (ByteBuffer) request.path(scrub("/" + db + (null == id ? "" : "/" + id))).method(GET)
                ...
                public void onRead(SelectionKey key) throws Exception {
                    if (null == cursor) {
                        ...
                        if (suffixMatchChunks(HEADER_TERMINATOR, response.headerBuf())) {
                            ...
                            if (d()) {
                                System.err.println(deepToString(response.statusEnum(), response, this, UTF8.decode((ByteBuffer) cursor
                                        .duplicate().rewind())));
                            }
                    ...
                    if (null == cursor) {
                        ...
                        if (-1 != read) {
                            ...
                            if (suffixMatchChunks(HEADER_TERMINATOR, response.headerBuf())) {
                                try {
                                    if (d())
                                        System.err.println(deepToString("??? ",

                                                UTF8.decode((ByteBuffer) flip.duplicate().rewind())));
                                    String header1 = ETag.getHeader();
                                    String str = response.dequotedHeader(header1);
                                ...
                                RelaxFactoryServer.App.get().getEXECUTOR_SERVICE().submit(new Callable() {
    ...
    DocPersist {
        public ByteBuffer visit(DbKeysBuilder dbKeysBuilder, ActionBuilder actionBuilder) throws Exception {
            ...
            actionBuilder.state().$req().headerString(Content$2dType, MimeType.json.contentType);
        ...
        public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder, final ActionBuilder actionBuilder) throws Exception {
            ...
            RelaxFactoryServer.App.get().enqueue(channel, OP_WRITE | OP_CONNECT, new Impl() {
                ...
                public void onWrite(SelectionKey key) throws Exception {
                    ...
                    header = (ByteBuffer) request.method(GET).path(scrub('/' + db + '/' + dbKeysBuilder.get(view))).headerString(Accept,
                ...
                public void onRead(SelectionKey key) throws IOException {
                                ...
                                case $200:
                                    ...
                                    if (response.headerStrings().containsKey(Content$2dLength.getHeader())) { //rarity but for empty rowsets
                                        ...
                                        if (cursor.remaining() == remaining) {
                                            ...
                                            //windows workaround?
                                            key.attach(new Impl() {
                                                private ByteBuffer cursor1 = cursor.capacity() > remaining ? (ByteBuffer) cursor.limit(remaining)
                    ...
                    //token suffix check
                    boolean suffixMatches = suffixMatchChunks(CE_TERMINAL, cursor.duplicate(), list.toArray(new ByteBuffer[list.size()]));
                    ...
                    RelaxFactoryServer.App.get().getEXECUTOR_SERVICE().submit(new Callable() {
                        public Object call() throws Exception {
                            ...
                            while (sum > 0 && src.hasRemaining()) {
                                ...
                                int i = Integer.parseInt(UTF8.decode((ByteBuffer) src.duplicate().flip()).toString().trim(), 0x10);
    ...
    JsonSend {
        public ByteBuffer visit(final DbKeysBuilder dbKeysBuilder, final ActionBuilder actionBuilder) throws Exception {
            ...
            HttpMethod method = 1 == slashCounter || !(lastSlashIndex < opaque.lastIndexOf('?') && lastSlashIndex != opaque.indexOf('/')) ? POST
                    : PUT;
            HttpRequest request = state.$req();
            ByteBuffer header = (ByteBuffer) request.method(method).path(opaque).headerInterest(STATIC_JSON_SEND_HEADERS)
                    .headerString(Content$2dLength, String.valueOf(outbound.length)).headerString(Accept, MimeType.json.contentType)
            ...
            RelaxFactoryServer.App.get().enqueue(channel, OP_WRITE | OP_CONNECT, new Impl() {
                ...
                HttpRequest request = actionBuilder.state().$req();
                private HttpResponse response;
                ByteBuffer header = (ByteBuffer) request.path(finalOpaque).headerInterest(STATIC_JSON_SEND_HEADERS)
    ...
    BlobSend {
        public ByteBuffer visit(DbKeysBuilder dbKeysBuilder, ActionBuilder actionBuilder) throws Exception {
            ...
            for (Object o : new Object[]{dbKeysBuilder.get(mimetypeEnum), dbKeysBuilder.get(mimetype), MimeType.bin}) {
                if (null != o) {
            ...
            final String sb = scrub('/' + db + '/' + docId + '/' + URLEncoder.encode(attachname, UTF8.displayName()) + "?rev=" + rev);

            final String ctype = x;
            final SocketChannel channel = createCouchConnection();
            final AtomicReference<ByteBuffer> res = new AtomicReference<ByteBuffer>();
            RelaxFactoryServer.App.get().enqueue(channel, OP_WRITE, new Impl() {
                @Override
                public void onWrite(SelectionKey key) throws Exception {

                    int limit = payload.limit();
                    ByteBuffer as = request.method(PUT).path(sb).headerString(Expect, "100-Continue").headerString(Content$2dType, ctype)
                            .headerString(Accept, MimeType.json.contentType).headerString(Content$2dLength, String.valueOf(limit)).as(
                                    ByteBuffer.class);
                    ...
                    switch (httpStatus) {
                        case $100:
                            key.interestOps(OP_WRITE).attach(new Impl() {
                                public HttpResponse response = (HttpResponse) request.$res().headerInterest(Content$2dLength);
                                public ByteBuffer cursor;
                                ...
                                public void onRead(SelectionKey key) throws Exception {
                                    ...
                                    response.apply(flip);
                                    finish = suffixMatchChunks(HEADER_TERMINATOR, response.headerBuf());
                                    if (finish) {
    ...
    public static final String[] STATIC_VF_HEADERS = Rfc822HeaderState.staticHeaderStrings(new HttpHeaders[]{ETag,
            Content$2dLength, Transfer$2dEncoding});
    //"premature optimization" s/mature/view/
    public static final String[] STATIC_JSON_SEND_HEADERS = Rfc822HeaderState.staticHeaderStrings(new HttpHeaders[]{ETag,
            Content$2dLength, Content$2dEncoding});
    //"premature optimization" s/mature/view/
    ...
    public static void main(String... args) throws NoSuchFieldException {
        ...
        @Language("JAVA") String s = "package rxf.server.gen;\n//generated\n  \n\nimport com.google.gson.FieldNamingPolicy;\n"
...
\n */\npublic interface CouchDriver{\n"
  ...
  private String writeParameterSetter(String rtypeTypeParams,String s1,etype etype,Class<?> clazz){
    @Language("JAVA") String y="public _ename_"+rtypeTypeParams+"  _name_(_clazz_ _sclazz_){parms.put(DbKeys.etype."
        +etype.name()+",_sclazz_);return this;}\n";
    s1+=y.replace("_name_",etype.name()).replace("_clazz_",clazz.getCanonicalName()).replace("_sclazz_",
        clazz.getSimpleName().toLowerCase()+"Param").replace("_ename_",name());
    return s1;
  }
