...
public class HttpProxyImpl extends Impl{
  ...
  public void onWrite(final SelectionKey browserKey) throws Exception{
    ...
    if(null==state){
      throw new Error("this GET proxy requires "+Rfc822HeaderState.class.getCanonicalName()+" in "
          +SelectionKey.class.getCanonicalName()+".attachments :(");
    }
    ...
    if(matcher.matches()){
      ...
      final SocketChannel couchConnection=BlobAntiPatternRelic.createCouchConnection();
      RelaxFactoryServer.App.get().enqueue(couchConnection,OP_CONNECT|OP_WRITE,new Impl(){
        @Override
        public void onRead(final SelectionKey couchKey) throws Exception{
          SocketChannel channel=(SocketChannel)couchKey.channel();
          final ByteBuffer dst=ByteBuffer.allocateDirect(BlobAntiPatternRelic.getReceiveBufferSize());
          ...
          couchKey.interestOps(OP_READ).attach(new Impl(){
            final ByteBuffer sharedBuf    =ByteBuffer.allocateDirect(Math.min(total,BlobAntiPatternRelic
            ...
            public void onRead(final SelectionKey couchKey) throws Exception{
              ...
              if(browserKey.isValid()&&remaining!=0){
                ...
                BlobAntiPatternRelic.recycleChannel(couchConnection);
              }
            }
          });
        }
