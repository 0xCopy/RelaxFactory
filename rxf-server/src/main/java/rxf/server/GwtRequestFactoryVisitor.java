...
public class GwtRequestFactoryVisitor extends Impl implements PreRead {
  ...
  public void onWrite(final SelectionKey key) throws Exception {
    if (payload == null) {
      ...
      EXECUTOR_SERVICE.submit(new Runnable() {
        ...
        public void run() {
          ...
          cursor = (ByteBuffer) UTF8.encode(payload).rewind();
          req.$res().headerString(HttpHeaders.Content$2dLength, String.valueOf(cursor.limit())).headerString(HttpHeaders.Content$2dType, MimeType.json.contentType);
    ...
    if (!cursor.hasRemaining()) key.interestOps(SelectionKey.OP_READ).attach(null);
  }
}