...
public class GwtRpcVisitor extends Impl implements PreRead,SerializationPolicyProvider{
  ...
  public void onWrite(final SelectionKey key) throws Exception{
    if(payload==null){
      key.interestOps(0);
      Future<?> submit=RelaxFactoryServer.App.get().getEXECUTOR_SERVICE().submit(new Runnable(){
        @Override
        public void run(){
          try{
            String reqPayload=UTF8.decode((ByteBuffer)cursor.rewind()).toString();

            RPCRequest rpcRequest=RPC.decodeRequest(reqPayload,delegate.getClass(),GwtRpcVisitor.this);

            try{
              payload=RPC.invokeAndEncodeResponse(delegate,rpcRequest.getMethod(),rpcRequest.getParameters(),rpcRequest
