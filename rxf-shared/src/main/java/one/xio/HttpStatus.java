 ...
 * does not expect a 100 (Continue) status message. Unexpected 1xx status responses MAY be ignored by a user agent. <br>
 ...
 * loops generate network traffic for each redirection. <br>
 * Note: previous versions of this specification recommended a maximum of five redirections. Content developers should
 * be aware that there might be clients that implement such a fixed limitation.
 * <p>
 * 4xx - Client Error <br>
 ...
 * display any included entity to the user. <br>
 * If the client is sending data, a server implementation using TCP SHOULD be careful to ensure that the client
 * acknowledges receipt of the packet(s) containing the response, before the server closes the input connection. If the
 * client continues sending data to the server after the close, the server's TCP stack will send a reset packet to the
 * client, which may erase the client's unacknowledged input buffers before they can be read and interpreted by the HTTP
 * application.
 * <p>
 * 5xx - Server Error <br>
...
public enum HttpStatus {
  ...
  /**
   * $2xx - Successful This class of status code indicates that the client's request was successfully received,
  ...
  /**
   * 3xx - Redirection This class of status code indicates that further action needs to be taken by the user agent in
   * order to fulfill the request. The action required MAY be carried out by the user agent without interaction with the
   * user if and only if the method used in the second request is GET or HEAD. A client SHOULD detect infinite
   ...
   * request method. User agents SHOULD display any included entity to the user. If the client is sending data, a server
   * implementation using TCP SHOULD be careful to ensure that the client acknowledges receipt of the packet(s)
   * containing the response, before the server closes the input connection. If the client continues sending data to the
   * server after the close, the server's TCP stack will send a reset packet to the client, which may erase the client's
   ...
   * is incapable of performing the request. Except when responding to a HEAD request, the server SHOULD include an
   * entity containing an explanation of the error situation, and whether it is a temporary or permanent condition. User
