...
public class DocFetchProxyImpl extends ContentRootImpl {
    ...
    public void onRead(final SelectionKey outerKey) throws Exception {
        ...
        if (null != getReq() && null != getMatchResults()) {
            ...
            new DocFetch().db("").docId(link).to().state().$res().addHeaderInterest(
                    Content$2dType);
            ...
            RpcHelper.EXECUTOR_SERVICE.submit(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        outerKey.interestOps(OP_WRITE).attach(
                                new FinishWrite(outerResponse.status(innerStatus).headerString(Content$2dType, innerResponse.headerString(Content$2dType)).headerString(Content$2dLength, innerResponse.headerString(Content$2dLength)).asByteBuffer(), outerPayload) { @Override public void onSuccess() { outerKey.interestOps(OP_READ).attach(null); } }); } catch (Exception e) { Errors.$500(outerKey); e.printStackTrace(); } } });
        } else {
            Errors.$500(outerKey);
        }
    }
