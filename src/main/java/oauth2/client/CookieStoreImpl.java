...
class CookieStoreImpl extends TokenStoreImpl {
  ...
  public native void set(String key, String value) /*-{
    $doc.cookie = @oauth2.client.CookieStoreImpl::COOKIE_PREFIX +
        encodeURIComponent(name) + '=' + encodeURIComponent(value);
  }-*/;

  @Override
  public native String get(String key) /*-{
    var m = @oauth2.client.CookieStoreImpl::ensureCookies();
    return m[@oauth2.client.CookieStoreImpl::COOKIE_PREFIX + key];
  }-*/;

  @Override
  public native void clear() /*-{
    var allCookies = @oauth2.client.CookieStoreImpl::ensureCookies()();
    var prefix = @oauth2.client.CookieStoreImpl::COOKIE_PREFIX;

    for (var key in allCookies) {
      if (key.indexOf(prefix) == 0) {
        this.@oauth2.client.CookieStoreImpl::set(*)(
            key.substring(prefix.length), '');
      }
    }
    @oauth2.client.CookieStoreImpl::cachedCookies = null;
  }-*/;
  ...
  private static native void loadCookies() /*-{
    @oauth2.client.CookieStoreImpl::cachedCookies = {};
    ...
    if (docCookie && docCookie != '') {
      ...
      for ( var i = 0; i < crumbs.length; ++i) {
        ...
        @oauth2.client.CookieStoreImpl::cachedCookies[name] = value;
      }
    }
  }-*/;
