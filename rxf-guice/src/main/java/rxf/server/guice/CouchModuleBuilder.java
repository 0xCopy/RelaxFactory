...
public class CouchModuleBuilder {
	...
	public CouchModuleBuilder withService(
			Class<? extends CouchService<?>> serviceType) {
	...
	public Module build() {
		return new AbstractModule() {
			...
			private <S extends CouchService<?>> void bindService(
					Class<S> serviceType) {
				assert hasAnEntity(serviceType);
				CouchServiceProvider<S> provider = new CouchServiceProvider<S>(
						serviceType);
