...
import static ds.server.SecurityImpl.DEAL_COUCH_LOCATOR;
import static ds.server.SecurityImpl.NPO_COUCH_LOCATOR;
import static ds.server.SecurityImpl.VENDOR_COUCH_LOCATOR;
...
public class DealSiteServer {
   ...
   * the MotdServlet
   *
   * registers a protocol level socket handler based on AsioVisitor interface.
   *
   * this one shows a non-http response to dump a file quick and dirty with no sanity checks
   *
   */
  static {
  ...
  public static void registerurlExprAsFirst(HttpMethod method, final Pattern passthroughExpr, final Class<? extends Impl> value) {
    LinkedHashMap<Pattern, Class<? extends Impl>> linkedHashMap = new LinkedHashMap<Pattern, Class<? extends Impl>>();
    linkedHashMap.put(passthroughExpr, value);
    Map<Pattern, Class<? extends Impl>> patternImplMap = NAMESPACE.get(method);
    if (null != patternImplMap) linkedHashMap.putAll(patternImplMap);
    ...
    if (System.getenv().containsKey("DEBUG_DEAL_EDITORS")) {
      ...
      BlobAntiPatternObject.EXECUTOR_SERVICE.schedule(new Runnable() {
        public void run() {
          ...
          List<Deal> lawnmower = service.findByProduct("lawnmower");
